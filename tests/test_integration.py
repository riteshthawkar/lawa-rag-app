...
class TestIntegration(unittest.TestCase):
    ...
    def test_rewrite_query_flow(self, mock_create):
        ...
        mock_completion1.choices[0].message.content = json.dumps({
            "action": "rewrite",
            "rewritten_query": "What are the types of visas available in the UAE?",
            "relevant_history_indices": []
        })
        
        mock_completion2 = MagicMock()
        mock_completion2.choices = [MagicMock()]
        mock_completion2.choices[0].message.content = "What are the types of visas available in the UAE and their application requirements?"
        
        mock_create.side_effect = [mock_completion1, mock_completion2]
        
        # Call the query rewriting agent and check the result
        async def run_test():
            result = await query_rewriting_agent("Tell me about visas", "en", [])
            self.assertEqual(result["action"], "rewrite")
            self.assertIn("visa", result["rewritten_query"].lower())
            self.assertIn("uae", result["rewritten_query"].lower())
    ...
    def test_out_of_scope_query_flow(self, mock_create):
        ...
        mock_completion.choices[0].message.content = json.dumps({
            "action": "respond",
            "response": "I'm sorry, but questions about iPhone repairs are outside my scope. I can only answer questions related to UAE governance, laws, economy, tourism, or other official matters."
        })
        mock_create.return_value = mock_completion
